...
const Job = struct {
    ...
    pub fn fix(self: Job, rules: []Rule) !void {
        var flag = true;
        while (flag) {
            flag = false;
            for (rules) |rule| {
                const a = std.mem.indexOfScalar(usize, self.pages, rule.a) orelse continue;
                const b = std.mem.indexOfScalar(usize, self.pages, rule.b) orelse continue;
                if (a < b) continue;
                const tmp = self.pages[a];
                self.pages[a] = self.pages[b];
                self.pages[b] = tmp;
                flag = true;
            }
        }
...
pub fn fail_sum_mid_pages(aa: std.mem.Allocator, input: []u8) !usize {
    const queue = try PrintQ.init(aa, input);
    var sum: usize = 0;
    var count: usize = 0;
    for (queue.jobs) |job| {
        // std.debug.print("checking job: {any}\n", .{job.pages});
        if (passes_rules(queue.rules, job)) continue;
        try job.fix(queue.rules);
        sum += job.pages[job.pages.len / 2];
        count += 1;
    }
    std.debug.print("{d} of {d} jobs fail\n", .{ count, queue.jobs.len });
    return sum;
}
...
test "p2" {
    ...
    const n = try fail_sum_mid_pages(aa, &input);
...
pub fn sln() !void {
    ...
    const sum = try fail_sum_mid_pages(aa, bytes);

    std.debug.print("Fails Mid Page Sum: {d}", .{sum});
}
