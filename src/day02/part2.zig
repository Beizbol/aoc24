...
fn is_safe(report: Report) bool {
    if (is_safe_skip(report, 0)) return true;
    if (is_safe_skip(report[1..], 0)) return true;
    for (1..report.len) |i| {
        if (is_safe_skip(report, i)) return true;
    }
    return false;
}

fn is_safe_skip(report: Report, skip: usize) bool {
    var prev: usize = report[0];
    const curr = if (skip == 1) report[2] else report[1];
    const desc = (prev > curr);

    for (report[1..], 1..) |lvl, i| {
        if (i == skip) continue;
        if (desc) {
